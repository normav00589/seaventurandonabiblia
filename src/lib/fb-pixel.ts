...
import { sendFbEvent, sendFbEvent2 } from "@/lib/fb-capi.functions";

export const FB_PIXEL_ID = "4377717859166211";
export const FB_PIXEL_ID_2 = "886009030499415";
...
export function trackFbEvent(
  ...
  // Pixel (browser) — dispara para todos os pixels inicializados
  if (typeof window.fbq === "function") {
    window.fbq("track", event, params ?? {}, { eventID: eventId });
  }

  // CAPI (server) — fire-and-forget para ambas as contas
  const payload = {
    eventName: event,
    eventId,
  ...
  try {
    void sendFbEvent({ data: payload }).catch(() => {});
  } catch {}
  try {
    void sendFbEvent2({ data: payload }).catch(() => {});
  } catch {}
}

/** Snippet oficial do Pixel — injetado uma única vez via <head>. Inicializa ambos os pixels. */
